...
def extract_summary(description):
    ...
    keywords = ["invention", "embodiment", "claims"]
    index = next((i for i, s in enumerate(sentences) if any(k in s.lower() for k in keywords)), None)
...
for i, url in enumerate(urls, 1):
    ...
    if USE_PLAYWRIGHT and (not filing_date or not publication_date or not expiration_date or not description):
        ...
        if not description:
            description = extract_description(soup_js)
